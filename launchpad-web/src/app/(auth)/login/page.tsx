...
import { cariApi, cariAuth, type UserRole } from '@/lib/cari-api';
...
export default function LoginPage() {
  ...
  const handleSignIn = async () => {
    ...
    try {
      setIsLoading(true);
      setFormError('');
      const data = await cariApi.login({ email, password });
      if (!data.session) throw new Error('Login did not return a session');
      cariAuth.setSession(data.session, data.user);
      localStorage.setItem('lp_onboarded', data.user.onboarded ? 'true' : 'false');
  ...
  const handleCreateAccount = async () => {
    ...
    try {
      setIsLoading(true);
      setFormError('');
      const role: UserRole = activeRole === 'company' ? 'COMPANY' : 'JOB_SEEKER';
      await cariApi.register({
        email,
        password,
        role,
        fullName: email.split('@')[0],
        companyName: role === 'COMPANY' ? `${email.split('@')[0]} Company` : undefined,
      });
      const data = await cariApi.login({ email, password });
      if (!data.session) throw new Error('Account created, but login did not return a session');
      cariAuth.setSession(data.session, data.user);
      localStorage.setItem('lp_onboarded', data.user.onboarded ? 'true' : 'false');
    ...
    <>
      ...
      <div className="min-h-screen flex flex-col lg:flex-row">
        ...
        >
          ...
          {formError && (
            <p style={{ fontSize: 13, color: '#FF4B4B', marginTop: 14, lineHeight: 1.4 }}>
              {formError}
            </p>
          )}
