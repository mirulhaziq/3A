...
export default function AnalysePage() {
  ...
  function handleAnalyse() {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem('lp_last_analysis', JSON.stringify({ score: MOCK_ANALYSIS.matchScore, timestamp: Date.now() }));
      ...
      const scoreInterval = setInterval(() => {
        count += 2;
        setScoreCount(count);
        if (count >= MOCK_ANALYSIS.matchScore) {
          clearInterval(scoreInterval);
          setScoreCount(MOCK_ANALYSIS.matchScore);
          if (MOCK_ANALYSIS.matchScore >= 75) {
            setTimeout(() => {
              confetti({
                particleCount: 100,
                spread: 80,
      ...
      const typeInterval = setInterval(() => {
        charIdx++;
        setVerdictText(MOCK_ANALYSIS.verdict.slice(0, charIdx));
        if (charIdx >= MOCK_ANALYSIS.verdict.length) {
    ...
    >
      ...
      <div
        style={{
          ...
          backgroundColor: '#FFFFFF',
          borderRadius: 20,
          border: MOCK_ANALYSIS.matchScore >= 75 ? '1px solid #D4F7B8' : '1px solid #E8E0D0',
          padding: 24,
        }}
      >
        <p
        ...
        <div style={{ display: 'flex', justifyContent: 'center', marginTop: 12 }}>
          <CuppyImage size="medium" state={MOCK_ANALYSIS.cuppyState} />
        </div>
      </div>
      ...
      >
        ...
        >
          {verdictText}
          {verdictText.length < MOCK_ANALYSIS.verdict.length && (
            <motion.span
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {activeResultTab === 'strengths' &&
              MOCK_ANALYSIS.strengths.map((item, index) => (
                <motion.div
                  key={item.title}
                  initial={{ opacity: 0, y: 8 }}
                  animate={{ opacity: 1, y: 0 }}
            ...
            {activeResultTab === 'gaps' &&
              MOCK_ANALYSIS.gaps.map((item, index) => (
                <motion.div
                  key={item.title}
                  initial={{ opacity: 0, y: 8 }}
                  animate={{ opacity: 1, y: 0 }}
            ...
            {activeResultTab === 'cvfixes' &&
              MOCK_ANALYSIS.cvFixes.map((item, index) => (
                <motion.div
                  key={index}
                  initial={{ opacity: 0, y: 8 }}
                  animate={{ opacity: 1, y: 0 }}
            ...
            {activeResultTab === 'keywords' && (
              <div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
                  {MOCK_ANALYSIS.missingKeywords.map((kw) => (
                    <span
                      key={kw}
                      style={{
                        backgroundColor: '#FFF5F5',
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {MOCK_ANALYSIS.presentKeywords.map((kw) => (
                    <span
                      key={kw}
                      style={{
                        backgroundColor: '#F9FFF5',
